...
Original file is located at
    https://colab.research.google.com/drive/1u80O5RBTI6Tvi68840pSmCfA9ALayi-Z
"""

API_KEY = "AIzaSyA5DP2TA82kWcbe8syZQQZWA4Ai2sUL58o" #google
API_KEY_Cloudfare = 'cfat_GAlrcJidnn4C1Pi4sVDyHx9nWU55PFcPEEeelnNo30bc7ce7' #cloudfare
Cloudfare_account_id = 'f92ea06819e6c326252476a8c30d453a'
...
# SUMMARY: Get env data at a point
def get_env_data(latitude, longitude, api_key = API_KEY):
    # Initialize the dictionary with default 0s as requested
    env_data = {
        'temp': 0, 'humidity': 0, 'wind_speed': 0,
        'pm25': 0, 'pm10': 0, 'no2': 0, 'o3': 0,
        'tree_pollen': 0, 'weed_pollen': 0, 'grass_pollen': 0
    }

    # ---------------------------------------------------------
    # 1. WEATHER from api.weather.gov (No API key required)
    # ---------------------------------------------------------
    headers = {"User-Agent": "EnvDataScript/1.0 (contact@example.com)"}
    try:
        points_url = f"https://api.weather.gov/points/{latitude},{longitude}"
        points_res = requests.get(points_url, headers=headers)
        points_res.raise_for_status()

        hourly_url = points_res.json()['properties']['forecastHourly']
        weather_res = requests.get(hourly_url, headers=headers)
        weather_res.raise_for_status()

        current_weather = weather_res.json()['properties']['periods'][0]

        env_data['temp'] = current_weather.get('temperature', 0)
        env_data['humidity'] = current_weather.get('relativeHumidity', {}).get('value', 0)

        wind_str = current_weather.get('windSpeed', '0')
        wind_match = re.search(r'\d+', wind_str)
        env_data['wind_speed'] = int(wind_match.group()) if wind_match else 0

    except Exception as e:
        print(f"Error fetching Weather.gov data: {e}")

    # ---------------------------------------------------------
    # 2. AQI from Google Air Quality API
    # ---------------------------------------------------------
    try:
        aqi_url = f"https://airquality.googleapis.com/v1/currentConditions:lookup?key={api_key}"

        # We must request extraComputations to get the raw pollutant values
        aqi_payload = {
            "location": {"latitude": latitude, "longitude": longitude},
            "extraComputations": ["POLLUTANT_CONCENTRATION"]
        }

        aqi_res = requests.post(aqi_url, json=aqi_payload)
        aqi_res.raise_for_status()

        pollutants = aqi_res.json().get('pollutants', [])

        for p in pollutants:
            # Google returns "PM2.5". This strips the decimal and makes it lowercase so it matches "pm25"
            code = p.get('code', '').lower().replace('.', '')

            if code in ['pm25', 'pm10', 'no2', 'o3']:
                env_data[code] = p.get('concentration', {}).get('value', 0)

    except Exception as e:
        print(f"Error fetching Google AQI data: {e}")

    # ---------------------------------------------------------
    # 3. POLLEN from Google Pollen API
    # ---------------------------------------------------------
...
# SUMMARY: send prompt to Nemotron
def prompt_with_nemotron(nemotron_prompt, chat_history=None):
    url = f"https://api.cloudflare.com/client/v4/accounts/{Cloudfare_account_id}/ai/run/@cf/nvidia/nemotron-3-120b-a12b"

    headers = {
        "Authorization": f"Bearer {API_KEY_Cloudfare}",
...
# SUMMARY: get route from orgin to destination
...
# SUMMARY: turn route into 5 seperate points to get env data at each point -> Formalize it
...
# SUMMARY: this will get user indent by understanding user response
...
def asking_response(user_prompt):
    nemotron_prompt = (
        "[SYSTEM ROLE]\n"
        "You are a Medical & Route Intelligence Agent. You just warned the user about "
        "environmental risks on a suggested route and asked if they accept the risk to proceed.\n\n"
...
# Ensure get_env_forecast is imported or defined above
...
{
  "origin": [44.5646, -123.2620],
  "destination": [44.5750, -123.2750],
  "transport": "walking",
  "user_data": {
    "age": 45,
    "asthma_severity": 2,
...

{
  ...
  "data": {
    ...
    "suggestion": [],
    "recommended_polyline": "encoded_string_here",
    "unchosen_polylines": ["encoded_string_2", "encoded_string_3"]
  }
}
...
{
  "status": "success",
  "message": "Conditions are excellent for your walk today! Proceed on Route 1. There is slightly elevated PM2.5, but it poses no risk to you. Safe travels!"
}
...

{
  "medical_logic": "...",
  "reasoning_summary": "...",
  "warnings": ["AQI is 150", "High Oak Pollen"],
...
{
  "status": "success",
  "message": "Based on your asthma profile, the high PM2.5 and oak pollen pose a significant risk. I strongly suggest waiting in a nearby library or cafe. Do you accept the risk and wish to proceed anyway?"
}
...
{
  "status": "success",
  "message": "In 2 hours, the temperature will be 70°F with 50% humidity. PM2.5 levels are elevated, and tree pollen is high. Because you have level 3 asthma and a tree pollen allergy, this combination presents a high risk for respiratory irritation. I recommend taking your prescribed antihistamine 30 minutes before heading out, carrying your rescue inhaler, and considering an N95 mask if you will be outdoors for an extended period."
}

# Test

origin = [45.5898, -122.5951] # Portland Airport area
destination = [45.5189, -122.6793] # Downtown Portland
transport = "driving"
user_data = {
    "age": 20,
    "asthma_severity": 3,
